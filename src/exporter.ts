...
*/
export async function importTable(table: string, entries: unknown[], tx?: IDBTransaction): Promise<number> {
  if (!db.connected)
    throw new ConnectionError('Database not connected')
...
*/
export async function exportDatabase(blacklist: string[] = []): Promise<Record<string, unknown[]>> {
  if (!db.connected)
    throw new ConnectionError('Database not connected')

  const exportData: Record<string, unknown[]> = {}
  const tables = Array.from(db.session.objectStoreNames).filter(table => !blacklist.includes(table))
...
export async function importDatabase(data: Record<string, unknown[]>): Promise<Record<string, number>> {
  if (!db.connected)
    throw new ConnectionError('Database not connected')

  const tx = db.session.transaction(Array.from(db.session.objectStoreNames), 'readwrite')

  const promises = []
  for (const table in data)
    promises.push(importTable(table, data[table], tx).then(n => [table, n]))

  return Promise.all(promises).then(results => Object.fromEntries(results) as Record<string, number>)
}
...
*/
export function exportDatabaseToBlob(): Promise<Blob> {
  return exportDatabase().then(objectToBlob)
}
